...
//-----------------------------------------------------------------------------
__global__ void kernelSleep(c64_t sleepTicks, c64_t* d_diff= NULL)
{
    c64_t start = clock64();
    c64_t offsetTicks = 0;
...
//-----------------------------------------------------------------------------
void Impl::sleep(int milliseconds, size_t streamIndex /*=EventStream*/)
{
...
//-----------------------------------------------------------------------------
ThrustPolicy Impl::thrustPolicy(size_t streamIndex /*=EventStream*/)
{
    // Keep index limited to either a color stream, or default event stream.
    if (streamIndex > EventStream) streamIndex = EventStream;
