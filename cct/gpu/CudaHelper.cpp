...
typedef std::map<int, ImplPtr> ImplPtrMap;
...
ImplPtr Helper::getImpl(int index)
{
    ...
    ImplPtr impl;
    ImplPtrMap::iterator it = implPtrMap.find(index);
    if (it == implPtrMap.end())
    {
        impl.reset(new Impl(device));
        implPtrMap.insert(std::make_pair(index, impl));
    }
    else
    {
        impl = it->second;
